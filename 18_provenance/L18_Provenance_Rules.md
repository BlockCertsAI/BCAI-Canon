...
### 18.5.1 Completeness
Canon must be able to reconstruct:  
**Origin → All Transformations → Present State**.  
The chain must be connected, acyclic (L15), and cryptographically sealed (L11).

### 18.5.2 Continuity
Each transformation event must reference parents, declare dependency type (L15), comply with mutability (L13), and satisfy temporal legality (L14). Any break invalidates the object.
...
The provenance system must support:
- backward tracing (present → origin)  
- forward tracing (origin → all derivatives, custody transitions, and effects)
...
## 18.13 Real-World Capability Enabled by Provenance Rules

Provenance rules enable **trust-by-construction across the real economy**.

They allow:
- Courts, regulators, and auditors to reconstruct the full origin and handling of any digital action, asset, or decision.
- Supply chains to prove authenticity, custody, and transformation of goods without relying on third-party attestations.
- Governments to issue, revoke, and audit credentials with permanent traceability.
- Enterprises to defend decisions, access changes, and transactions with cryptographic evidence.
- AI systems to operate with explainable decision ancestry rather than opaque outputs.

Provenance turns digital systems from **records of activity** into **records of responsibility**.
